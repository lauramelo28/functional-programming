...
let result1 = randomNumbers.filter(number => number % 2 == 1 )
console.log(result1);
...
let result2 = crazyNumbers.reduce((counter, number)=>{
    return (number < 501 ? counter+1 : counter)
},0);
console.log(result2);
...
let result3 = numbers.map(number => number * number)
console.log(result3)
...
let result4 = tarantinoMovies.filter(film => film.release < 2000)
console.log(result4);
...
let result5 = tvShows.map(show => show.name);
console.log(result5)
...
let result6 = cart.reduce((acumulator, name) => { return `${acumulator}-${name.name}\n`},"" )
console.log(result6)